...
|---|---|---|
| `ci.yml` | push/PR → `master` | Build + test .NET (then validate the evaluation dataset via `--validate-only`), lint/typecheck/build frontend, and typecheck notification-service — all in parallel |
