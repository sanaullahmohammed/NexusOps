...
**Decision**: AgentHost holds a MassTransit `IRequestClient<InvestigateOrderRootCause>`, created per call via `IClientFactory.CreateRequestClient<InvestigateOrderRootCause>(RequestTimeout.After(s: 12))` in `OrderTools.cs` — not `AddRequestClient` in `Program.cs`, since `OrderTools` is a singleton and `IRequestClient<T>` itself is scoped; `IClientFactory` is the singleton-safe way to mint one on demand. The `investigate_order_root_cause` tool handler calls `await client.GetResponse<RootCauseInvestigationResult>(command, cancellationToken)`. The 12s figure is not arbitrary: `InvestigationFanOutConsumer` runs the order lookup first (its own 5s timeout), then inventory and product in parallel (another 5s timeout each) — worst case 10s before the saga can even finalize. A client-side timeout below that worst case (an earlier draft used 8s) would make the caller see "investigation timed out" for exactly the slow-but-not-actually-broken case the `Degraded` result exists to handle correctly; 12s leaves headroom above the 10s worst case for message transit and serialization. The saga does **not** use the `RespondAsync` sugar (that only works from within the consume context that received the original request) — instead, on consuming `InvestigateOrderRootCause`, it captures `ResponseAddress` and `RequestId` from the message headers into its own persisted state, and when it later finalizes (triggered by a `ProductFindingReported`/`InventoryFindingReported`/`OrderFindingReported` consume context — a different context entirely), it resolves a send endpoint for the stored `ResponseAddress` and sends `RootCauseInvestigationResult` with `RequestId` set explicitly, which MassTransit's request client correlates back to the original caller's pending `Task`.

**Rationale**: This is the most direct implementation of the roadmap's literal instruction ("AgentHost publishes its command and awaits the saga result"). It keeps AgentHost's involvement in the bus to a thin, transport-level concern (a request client), never a consumer or state machine, honoring Constitution Principle I. It also gives FR-011 (late responses discarded) for free: with a bus-scoped client from `IClientFactory.CreateRequestClient`, the reply address is AgentHost's own bus endpoint (which lives for the process's lifetime, not a per-call queue), so a late response does land there, but it is discarded because no pending request matches its `RequestId` — a silent no-op from the caller's perspective, with no explicit "is this too late?" check needed anywhere.
...
**Decision**: `OrderInvestigationSagaState` is persisted via MassTransit's EF Core saga repository against a dedicated `OrderInvestigationDbContext`, using `ConcurrencyMode.Optimistic` with a `RowVersion` (`uint`, mapped to Postgres's `xmin` system column via `.IsRowVersion()` — Npgsql maps `uint` onto `xmin` directly; a `byte[]` shadow property is the SQL Server/`rowversion` convention, not the Postgres one) concurrency token column. On a concurrency conflict (`DbUpdateConcurrencyException`), MassTransit's EF Core repository automatically retries the consume from a fresh read of the row — no custom retry code needed in the saga itself.
