...
- What happens when the conversation store is temporarily unavailable? → The request proceeds as a stateless exchange; no history is loaded or saved, and no error is returned to the caller. The caller receives a normal 200 response with no indication of degradation. **The caller-supplied session ID is preserved and echoed back unchanged** — an unreachable store is not evidence that the session is gone, so FR-007's minting rule does not apply. See FR-013.
...
- **FR-007**: When a request arrives with an expired, unknown, or malformed session ID, the system MUST start a fresh session and return a newly minted session ID rather than returning an error to the caller. This applies only when the conversation store was successfully consulted and reported no such session. If the store could not be reached, whether the session exists is unknown and FR-007 MUST NOT be applied — see FR-013.
- **FR-013**: When the conversation store is unreachable, the system MUST preserve the caller-supplied session identifier and process the request statelessly. It MUST NOT mint a replacement identifier, because a failed lookup is not evidence that the session is absent. A request that supplies no identifier still mints one. *(Added by feature 003, FR-009.)*
- **FR-014**: A request that supplies no session identifier MUST NOT query the conversation store, and MUST NOT emit a `session.history_loaded` event. The identifier is generated in-process, so a read is a guaranteed miss and the event describes a load that never occurred. *(Added by feature 003, FR-010.)*
- **FR-015**: When the agent invocation fails, the response MUST carry the active session identifier, so the user turn persisted under FR-005 is reachable by the caller. *(Added by feature 003, FR-011.)*
- **FR-016**: Conversation-store availability MUST NOT gate the service's readiness. Since FR-010 and FR-013 require the service to keep serving when the store is unreachable, a readiness probe that fails on store unavailability would remove the service from rotation precisely when it is still able to answer. *(Added by feature 003, FR-021.)*
- **FR-017**: A supplied session identifier MUST be validated against the published format (canonical hyphenated UUID) before use. A value failing validation is malformed and MUST be handled under FR-007. Unvalidated input MUST NOT be used as a store key, echoed back to the caller, or written to logs. *(Added by feature 003, FR-022.)*
- **FR-018**: Persisting conversation turns MUST NOT be cancelled by the caller's request being aborted. FR-005's guarantee that the user turn survives a failed agent invocation is void if the write is cancelled by the same disconnect that caused the failure. *(Added by feature 003, FR-023.)*
- **FR-008**: The system MUST cap the number of turns retained per session at a configurable maximum (`MaxTurns`). A turn is a single message (user OR assistant), not a user+assistant pair; `MaxTurns: 20` accommodates 10 complete exchanges. Excess turns are dropped oldest-first in a single pass whenever the total exceeds `MaxTurns`, regardless of how many turns were appended in the current request. If `MaxTurns` **or** `SlidingExpirationMinutes` is configured to 0 or a negative value, the system MUST fail at startup with a descriptive configuration error naming the offending key. *(Extended by feature 003, FR-008: the original requirement covered `MaxTurns` only, so a non-positive `SlidingExpirationMinutes` started cleanly and then threw on every store write, where it was swallowed and misreported as a connection failure.)*
...
- **FR-012**: The system MUST emit a structured log event for each of the following session lifecycle moments, with the specified log level and required fields. Every event MUST identify the session with the same derived token, so that events emitted by different components can be correlated; the token MUST NOT expose a recoverable portion of the raw identifier. *(Token consistency added by feature 003, FR-012: the two emitting components previously derived the field differently — one hashed the identifier, the other emitted its first eight raw characters — so no query could join them.)*
  ...
  - `session.degraded` **(Warning)**: `sessionId`, `errorCategory` (one of: `connection`, `timeout`, `serialisation`, `configuration`), `historyLoadedBeforeFailure` (bool), `turnCountLoaded` (int; 0 if load failed before any turns were read), `timestamp`
